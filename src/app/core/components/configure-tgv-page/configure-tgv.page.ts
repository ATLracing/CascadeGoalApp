...
import { get_dual_week_str, contains } from 'src/app/core/providers/discrete-date';
import { ChangeWeekComponent } from '../change-week/change-week.component';
...
@Component({
  ...
  templateUrl: 'configure-tgv.page.html',
  styleUrls: ["configure-tgv.page.scss"]
})
export class ConfigureTgvPage {
  ...
  async associate()
  {
    const modal = await this.modal_controller_.create({component: AssociateComponent, componentProps: { is_task: this.tgv_node_.type == InflatedRecord.Type.TASK } });
